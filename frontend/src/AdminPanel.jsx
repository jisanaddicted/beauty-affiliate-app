...
import { 
  ...
  Truck, CheckCircle2, RefreshCw, Search, ShoppingBag 
} from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const handleMarkAsDelivered = async (order, index) => {
    ...
    try {
      ...
      if (response.data.result === 'success') {
        // Safe context state array mutations for speedy UI update feedback
  ...
  return (
    <div className="min-h-screen flex bg-neutral-50 text-neutral-900">
      ...
      <aside className="w-64 bg-white border-r border-neutral-200 p-6 flex flex-col justify-between shrink-0">
        <div>
        ...
        {activeTab === 'orders' && (
          <div className="space-y-6 animate-fadeIn">
            <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <div>
                <h1 className="text-2xl font-black tracking-tight">Order Synchronization Desk</h1>
                <p className="text-sm text-neutral-500 mt-1">Cross-reference active creator pipelines and checkmark packages to release balances.</p>
              </div>
              <button 
                type="button" onClick={fetchAllOrders} disabled={loadingOrders}
                className="flex items-center justify-center gap-1.5 bg-neutral-200/70 hover:bg-neutral-200 text-neutral-800 font-bold text-xs px-4 py-2.5 rounded-xl transition-all disabled:opacity-50 shrink-0"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white border border-neutral-200 p-4 rounded-xl flex items-center gap-3 shadow-sm">
                ...
                <div>
                  ...
                  <span className="text-base font-black text-neutral-950">{totalOrders} Submissions</span>
                </div>
              </div>
            ...
            <div className="bg-white border border-neutral-200 rounded-2xl overflow-hidden shadow-sm">
              ...
              {/* TOOLBAR */}
              <div className="p-4 border-b border-neutral-100 flex flex-col md:flex-row justify-between items-stretch md:items-center gap-3 bg-neutral-50/30">
                ...
                <div className="flex bg-neutral-100 p-1 rounded-xl text-xs font-bold self-start md:self-auto">
                  {['All', 'On Way', 'Delivered'].map((filter) => (
                    <button
                      key={filter} type="button" onClick={() => setStatusFilter(filter)}
                      className={`px-3 py-1.5 rounded-lg transition-all ${statusFilter === filter ? 'bg-white text-neutral-950 shadow-sm' : 'text-neutral-500 hover:text-neutral-900'}`}
                    >
                      {filter}
                    </button>
                  ))}
              ...
              {loadingOrders ? (
                <div className="p-16 text-center text-xs text-neutral-400 font-bold tracking-wide animate-pulse">Pulling live Google Sheet entries...</div>
              ) : filteredOrders.length === 0 ? (
                <div className="p-16 text-center text-xs text-neutral-400 font-semibold">No order rows matched your filter query.</div>
              ) : (
                          ...
                          <tr key={idx} className="hover:bg-neutral-50/30 transition-colors">
                            ...
                            <td className="px-6 py-4 text-right">
                              {isDelivered ? (
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleMarkAsDelivered(order, idx)} 
